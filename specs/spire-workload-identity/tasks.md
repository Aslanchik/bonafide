...
- Add `pyspiffe` to `sdks/agent-py/pyproject.toml` and remove `PyJWT[crypto]` from the agent SDK's dependencies if it is no longer used elsewhere in `bonafide_agent/` (the resource SDK still depends on it for task-token verification; the agent SDK does not, after pyspiffe takes over actor_token minting). The original spec named `python-jose[cryptography]` — that pin was swapped to PyJWT mid-TEC, see `agent-notes.md` 2026-06-04.
